...
# def run(Y,pi0,p0,q0,ite = 200):
#     U = E_step(pi0,p0,q0,Y)
#     [pi_temp,p_temp,q_temp] = M_step(U,Y)
#     llh_temp = likelihood(pi_temp,p_temp,q_temp,Y)
#     print('step 0 :',pi_temp,p_temp,q_temp,'\nLikelihood: ', llh_temp)
#     for i in range(ite):
#         U = E_step(pi_temp,p_temp,q_temp,Y)
#         delta = [pi_temp-M_step(U,Y)[0],p_temp-M_step(U,Y)[1],q_temp-M_step(U,Y)[2]]
#         if np.linalg.norm(delta, 2) > 1e-6:
#             [pi_temp,p_temp,q_temp] = M_step(U,Y)
#             llh_temp = likelihood(pi_temp,p_temp,q_temp,Y)
#             print('step',i+1, ':',pi_temp,p_temp,q_temp,'\nLikelihood: ', llh_temp)
#         else:
#             break
        
#     return pi_temp,p_temp,q_temp