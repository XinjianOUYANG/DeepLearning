...
# 生成最小二乘回归树（least squares regression tree）
class RegressionCART:
    ...
    def build(self, X, Y, depth=1):
        ...
        if self.verbose:
            ...
            pprint(X)
            print(Y)
        best_se = inf # inf: infinite，正无穷
        best_col, best_val = -1, nan  # nan: nan代表Not A Number（不是一个数），它并不等于0
        ...
        if depth < self.max_depth and len(set(Y)) > 1:
            for col in range(len(X[0])):
                for val in X[:, col]:
                    # Don't split by the maximum value
                    # because no value is larger than it
                    # so the right part is empty
                    if val != X[:, col].max():  # ??
